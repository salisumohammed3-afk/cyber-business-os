  ...
  return (
    <div className="w-80 border-l border-border bg-card flex flex-col h-full">
        ...
        {messages.map((msg) => (
          <div key={msg.id} className={`flex gap-2 ${msg.role === "user" ? "flex-row-reverse" : ""}`}>
            <div className={`w-5 h-5 rounded-sm flex items-center justify-center flex-shrink-0 mt-0.5 ${
              msg.role === "user" ? "bg-emerald/20" : "bg-muted"
            }`}>
              {msg.role === "user" ? (
                <User size={10} className="text-emerald" />
              ) : (
      ...
      <div className="p-3 border-t border-border">
        <div className="flex items-center gap-2 bg-background border border-border rounded-sm p-1.5">
          <input
            value={input}
          ...
          <button
            onClick={handleSend}
            className="p-1 rounded-sm hover:bg-emerald/10 text-muted-foreground hover:text-emerald transition-colors"
          >
            <Send size={12} />
          </button>
        </div>
